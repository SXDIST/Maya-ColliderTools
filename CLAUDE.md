...
This creates a `ColliderTools` shelf with quick buttons for `UBX Box`, `UBX OBox`, and procedural history actions for `UCX Hull`, `UCX Cylinder`, and `UCP Capsule`. The Python UI attempts to load `build-vs2026-devkitbase-env/Release/ColliderTools.mll` automatically if the plugin is not already loaded.
...
The top-level `CMakeLists.txt` builds one plugin target named `ColliderTools`. It uses Maya's `pluginEntry.cmake`, links `OpenMaya`, `OpenMayaUI`, and `Foundation`, and compiles both the new collider command code and selected legacy DDConvexHull sources.

Plugin registration is centralized in `src/DDConvexHullPlugin.cpp`. It registers:

- dependency node `DDConvexHull`, implemented by `src/DDConvexHullNode.cpp`
...
- uses `DDConvexHullUtils::componentToVertexIDs` for component selections
...
Convex hull generation reuses the bundled MIT-licensed DDConvexHull/StanHull code integrated under `src/`:

- `src/DDConvexHullUtils.cpp` exposes `DDConvexHullUtils::generateMayaHull()`
- `src/StanHull/hull.cpp` contains the underlying hull algorithm
- `src/DDConvexHullNode.cpp` remains available for node-based/procedural hull workflows, supports component-list inputs, cleans unique points before hull generation, and exposes `maxVertices`
